...
class BackendProxyHttpClient(
    ...
    override fun execute(
        ...
        // Transform the request for our backend
        ...
        okHttpClient.newCall(okHttpRequest).execute().use { okHttpResponse ->
            ...
            // Convert OkHttp response to SDK HttpResponse
    ...
    private fun transformRequest(request: HttpRequest): TransformedRequest {
        ...
        // Map Anthropic endpoints to backend endpoints
        val newUrl = when {
        ...
        val body = request.body?.let { requestBody ->
            // Convert HttpRequestBody to ByteArray
    ...
    private fun buildOkHttpRequest(transformedRequest: TransformedRequest): Request {
        ...
        // Add user's Anthropic API key if available
    ...
    /**
     * Custom HttpResponse implementation that wraps our backend response
     */
