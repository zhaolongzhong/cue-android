...
class WebSocketManager(
                    ...
                    when (event) {
                        ...
                        is WebSocketEvent.Error -> {
                            ...
                            _connectionState.value = ConnectionState.Error(
                                ConnectionError.ConnectionFailed(event.error.message ?: "Unknown error")
                            )
                            if (shouldReconnect) {
    ...
    fun send(message: String, recipient: String): Boolean {
        if (_connectionState.value != ConnectionState.Connected) {
            Log.e(TAG, "Attempting to send message while not connected. State: ${_connectionState.value}")
            return false
        }
        ...
        try {
            ...
            when (type) {
                ...
                "client_connect", "client_disconnect", "client_status" -> {
                    ...
                    val clientStatus = when {
                        ...
                        type == "client_disconnect" -> {
                            val existingStatus = _clientStatuses.value.find { it.id == otherClientId }
                            ClientStatus(
                                id = otherClientId,
                ...
                "assistant", "user" -> {
                    ...
                    Log.d(TAG, "Received message(id:${messagePayload.optString("msg_id")}): ${messagePayload.optString("message")}")
                    _messages.emit(text)
                }
                "generic", "error" -> {
                    val payload = eventMessage.getJSONObject("payload")
